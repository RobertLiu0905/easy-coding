...
function parseIndentationContext(inputLines, inputLinesForDetection, context, config) {
    ...
    if (!isCurrentLineInCompletionBlank && !isCurrentLineInPrefixBlank) {
        ...
        if (config.experimentalKeepBlockScopeWhenCompletingLine) {
            ...
            result.allowClosingLine && (result.allowClosingLine = isBlockOpeningLine(inputLinesForDetection, 0));
        }
    }
    ...
    if (firstNonBlankLineInSuffix < suffixLines.length) {
        ...
        result.allowClosingLine && (result.allowClosingLine = getIndentationLevel(firstNonBlankLineInSuffixText) < result.indentLevelLimit);
    }
    return result;
}
export function limitScopeByIndentation(config) {
    return (input, context) => {
        ...
        if (index < inputLines.length) {
            logger.debug({ inputLines, prefixLines, suffixLines, scopeLineCount: index }, "Remove content out of indent scope");
