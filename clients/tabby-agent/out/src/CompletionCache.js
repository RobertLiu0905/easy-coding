...
class CompletionCache {
    constructor() {
        this.logger = (0, logger_1.logger)("CompletionCache");
        this.options = {
            maxCount: 10000,
            prebuildCache: {
                enabled: true,
            ...
            for (const choice of value.choices) {
                ...
                for (const position of perLinePositions) {
                    ...
                    for (const prefix of [completionTextPrefix, ...completionTextPrefixWithAutoClosedChars]) {
                        const entry = {
                            key: new CompletionContext_1.CompletionContext({
                                ...key,
    ...
    getPerLinePositions(completion) {
        ...
        const lines = (0, utils_1.splitLines)(completion);
        let index = 0;
        let offset = 0;
    ...
    getPerCharacterPositions(completion) {
        const result = [];
        const option = this.options.prebuildCache;
        const text = (0, utils_1.splitLines)(completion).slice(0, option.perCharacter.lines).join("");
        let offset = 0;
