...
use crate::service::wct_customized_logic::{verify_token, VerifyTokenResult};
...
impl ServerContext {
    ...
    async fn authorize_request(&self, request: &Request<Body>) -> (bool, Option<ID>) {
            ...
            match verify_token(token).await {
                Ok(result) => {
                    ...
                    if result.need_register {
                        ...
                        // 通过邮箱做唯一性约束
                        let email_prefix = String::from(user_code.to_string());
                        let email_suffix = "@iwhalecloud.com";
                        let email = email_prefix + email_suffix;
                        return match self.auth.register_by_wct(email, "123456".to_string(), token.to_string(), user_id, user_code.clone(), user_name)
                            .await {
                            Ok(id) => {
                                (true, Some(juniper::ID::new(user_code)))
                            }
                            Err(e) => {
                                (false, None)
                            }
        ...
        match self
            ...
            .await
        {
            Ok(id) => (true, Some(juniper::ID::new(id))),
            Err(_) => (false, None),
        }
    }
}
...
pub fn graphql_pagination_to_filter(
    ...
    match (first, last) {
        (Some(first), None) => {
            let after = after.map(|x| ID::new(x).as_rowid()).transpose()?.map(|x| x as i32);
            Ok((Some(first), after, false))
        }
        (None, Some(last)) => {
            let before = before.map(|x| ID::new(x).as_rowid()).transpose()?.map(|x| x as i32);
