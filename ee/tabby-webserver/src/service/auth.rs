...
impl AuthenticationService for AuthenticationServiceImpl {
    ...
    // liulin
    async fn register_by_wct(&self, email: String,
                             password1: String,
                             auth_token: String,
                             user_id: i64,
                             user_code: String,
                             user_name: String) -> Result<i64> {
        let id = self.db.create_user_wct(email, password1, auth_token.clone(), user_id, user_code, user_name).await?;

        self.db.update_hash_id(id.as_id().to_string(), id).await;

        // 暂时不需要refresh token
        // let refresh_token = self.db.create_refresh_token(user_id).await?;

        // let resp = RegisterResponse::new(auth_token, "".to_string());
