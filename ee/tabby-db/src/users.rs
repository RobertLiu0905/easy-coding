...
impl DbConn {
    ...
    // liulin
    pub async fn create_user_wct(&self,
        email: String,
        password: String,
        token: String,
        user_id: i64,
        user_code: String,
        user_name: String) -> Result<i64> {
        let mut transaction = self.pool.begin().await?;
        let res = query!("INSERT INTO users (email, password_encrypted, is_admin, auth_token, user_code, user_name) VALUES (?, ?, ?, ?, ?, ?) ON CONFLICT(email) DO UPDATE SET auth_token = excluded.auth_token",
            email, password, false, token, user_code, user_name)
    ...
    pub async fn update_hash_id(&self, hash_id: String, id: i64) -> Result<(), sqlx::Error> {
        let mut transaction = self.pool.begin().await?;
        query!("UPDATE users SET hash_id = ? WHERE id = ?", hash_id, id)
            .execute(&mut *transaction)
            .await?;
        transaction.commit().await?;
        Ok(())
    ...
    pub async fn verify_auth_token(&self, token: &str, requires_owner: bool) -> Result<String> {
        ...
        let Some(id) = query_scalar!(
            "SELECT user_code FROM users WHERE auth_token = ? AND active AND (id == ? OR NOT ?)",
            token,
            OWNER_USER_ID,
            requires_owner
        )
